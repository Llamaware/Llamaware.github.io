...
Yanfly.Parameters = {
    ...
    "ScrollBarColor": "0",
	"ScrollBarSpriteCode": "var padding=backlogWindow.standardPadding(),width=padding/2,height=Graphics.boxHeight,visibleRows=backlogWindow.height-2*padding;(visibleRows=Math.floor(visibleRows/backlogWindow.lineHeight()))<backlogWindow.maxItems()&&(height*=visibleRows/Math.max(1,backlogWindow.maxItems()));var max=Math.floor(Math.max(1,backlogWindow.maxItems())/backlogWindow.maxCols())-1;this._increment=Graphics.boxHeight/Math.max(1,max),this.bitmap=new Bitmap(width,height),this.bitmap.fillAll(backlogWindow.textColor(scrollBarColor));",
	"ScrollSpeed": "4",
    "ScrollWrap": "true",
    "BackgroundType": "1",
    "BackOpacity": "192",
...
Yanfly.Param.MsgBacklogScrBarSpriteCode = Yanfly.Parameters['ScrollBarSpriteCode'];
